...
export default function DIDsPage() {
  ...
  return <div className="space-y-8">
    ...
    {unlockingDid && <div className="fixed inset-0 z-20 flex items-center justify-center bg-black/70 p-4"><div className="w-full max-w-md rounded-2xl border border-zinc-700 bg-zinc-900 p-6 shadow-2xl"><div className="flex items-start justify-between"><div><h2 className="text-xl font-semibold text-zinc-100">Unlock DID</h2><p className="mt-2 break-all font-mono text-xs text-zinc-400">{unlockingDid}</p></div><button aria-label="Close" title="Close" onClick={resetUnlockForm} className="text-zinc-500 hover:text-zinc-200"><X size={18} /></button></div><label className="mt-6 block text-sm text-zinc-300">Passphrase <input type="password" autoFocus value={unlockPassphrase} onChange={(event) => setUnlockPassphrase(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void unlockSelectedDid(); }} className="mt-2 w-full rounded-lg border border-zinc-700 bg-zinc-950 px-3 py-2.5 text-sm outline-none focus:border-emerald-400" placeholder="Enter your DID passphrase" /></label>{unlockError && <p className="mt-4 text-sm text-red-300">{unlockError}</p>}<button onClick={() => void unlockSelectedDid()} className="mt-6 w-full rounded-lg bg-emerald-400 px-4 py-3 text-sm font-semibold text-zinc-950 hover:bg-emerald-300">Unlock identity</button></div></div>}
  </div>;
}
