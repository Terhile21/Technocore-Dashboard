...
function introLines(pack: ProofPack): string[] {
  ...
  return [`- Room: ${item.room}`, `- Sequence: ${item.sequence ?? "pending"}`, `- Nonce: ${item.nonce}`, `- Time: ${item.timestamp}`, `- Text: ${item.text}`];
}
function contributionLines(item: NonNullable<ProofPack["primaryContribution"]>): string[] { return [`- URL: ${item.publicUrl}`, `- Description: ${item.description}`, `- Room: ${item.room}`, `- Sequence: ${item.sequence ?? "pending"}`, `- Nonce: ${item.nonce}`, `- Time: ${item.createdAt}`, `- Signed text: ${item.text}`]; }

export function formatShortProof(pack: ProofPack): string {
  return [`Technocore proof`, `DID: ${pack.did}`, `Fingerprint: ${pack.fingerprint}`, `Lobby intro: ${pack.lobbyIntro ? `room ${pack.lobbyIntro.room}, sequence ${pack.lobbyIntro.sequence ?? "pending"}` : "not recorded"}`, `Contribution: ${pack.primaryContribution?.publicUrl ?? "not recorded"}`, `Signed record: ${pack.primaryContribution ? `room ${pack.primaryContribution.room}, sequence ${pack.primaryContribution.sequence ?? "pending"}` : "not recorded"}`].join("\n");
}
export function formatShareProof(pack: ProofPack): string {
  return [`Recorded a Technocore contribution.`, ``, `DID: ${pack.did}`, `Lobby: ${pack.lobbyIntro ? `room ${pack.lobbyIntro.room}, sequence ${pack.lobbyIntro.sequence ?? "pending"}` : "not recorded"}`, `Contribution: ${pack.primaryContribution?.publicUrl ?? "not recorded"}`, `Signed Technocore record: ${pack.primaryContribution ? `room ${pack.primaryContribution.room}, sequence ${pack.primaryContribution.sequence ?? "pending"}` : "not recorded"}`].join("\n");
}
